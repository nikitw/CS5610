...
services.factory('lfmAPIservice', function($http) {
    ...
    lfmAPI.getAllAlbums = function(filter, callback) {
        $http.jsonp(URL+"albums?callback=JSON_CALLBACK&keyword="+filter.keyword)
            .success(function(data){
                albums = [];
                for(var dt in data.data) {
                    data.data[dt].dor = new Date(data.data[dt].dor).toDateString();
                    albums.push(data.data[dt]);
                }
                callback(albums);
            });
    };

    lfmAPI.getAllSongs = function(filter, callback) {
        $http.jsonp(URL+"songs?callback=JSON_CALLBACK&keyword="+filter.keyword)
            .success(function(data){
                songs = [];
                for(var dt in data.data) {
                    data.data[dt].dor = new Date(data.data[dt].dor).toDateString();
                    songs.push(data.data[dt]);
                }
                callback(songs);
            });
    };
