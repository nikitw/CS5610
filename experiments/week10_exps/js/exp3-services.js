...
services.factory('mapsAPIservice', function($http) {
    ...
    mapsAPI.load = function () {
        getLocation(function(data) {
            ...
            if(data.err)
                ...
                mapsAPI.plotMarkers(initialize(null, loc, 15));
            }
        });
    };

    mapsAPI.plotMarkers = function (map) {
        for(var m in markers) {
            ...
            new google.maps.Polyline({
                path: [markers[m].position, markers[n].position],
                strokeColor:"#99f",
                strokeOpacity:0.8,
                strokeWeight:2,
                map: map
    ...
    mapsAPI.search = function (address) {
        var address = address.replace(/\s+/, '+');
        if(address == '')
            initialize();
        else
